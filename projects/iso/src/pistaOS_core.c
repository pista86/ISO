...
static uint32_t get_running_task_index(void);
...
void SysTick_Handler(void) {
	...
	for (int i = 0; i < PISTAOS_MAX_TASK; i++) {
		if (taskList[i].status == WAITING && taskList[i].delayTicks > 0) {
...
void taskDelay(uint32_t delayTicks) {
	...
	taskList[get_running_task_index()].status = WAITING;
...
uint32_t get_next_context(uint32_t current_sp) {
	...
	if (ready_task_index != currentTaskIndex) // Tarea READY de mayor prioridad es distinta a la que se ejecuta?
	{
		...
		else
		{
			...
			if (taskList[currentTaskIndex].status == RUNNING)
			{
				// Si la tarea anterior estaba corriendo pasa a ready, si fue suspendida por un taskDelay ya está en waiting y no se cambia el estado
				taskList[currentTaskIndex].status = READY;
			}
		}
		...
		if (ready_task_index == -1) {
			...
			taskList[currentTaskIndex].status = RUNNING;
